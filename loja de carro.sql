...
create view A as SELECT c.nome as "Nome do cliente", v.modelo as "Modelo do Veículo"
FROM Aluguel a 
join cliente c on c.id_cliente = a.id_cliente
join Veiculo v on v.id_veiculo = a.id_veiculo;

create view B as SELECT c.nome as "Nome do cliente", v.modelo as "Modelo do Veículo", a.valor_total as "Valor total do aluguel"
from aluguel a 
join Cliente c on c.id_cliente = a.id_cliente
join Veiculo v on v.id_veiculo = a.id_veiculo
where valor_total > 1000.00;

create view C as SELECT v.modelo as "Modelos nunca alugados"
from Veiculo v
left join Aluguel a on v.id_veiculo = a.id_veiculo
where a.id_veiculo is null;

create view  D as SELECT c.nome as "Nome do Cliente", v.categoria as "Categoria do veículo", c.email as "Email do Cliente"
from Aluguel a
...
create view I as 
SELECT EXTRACT(YEAR FROM a.data_fim) AS ano, EXTRACT(MONTH FROM a.data_fim) AS mes,SUM(a.valor_total) AS "Faturamento"
FROM Aluguel a
GROUP BY EXTRACT(YEAR FROM a.data_fim), EXTRACT(MONTH FROM a.data_fim)
ORDER BY ano, mes;

create view J as 
SELECT c.nome as "Nome do Cliente", v.modelo as "Modelo do veículo", (a.data_fim - a.data_inicio) as "Período do Aluguel (em dias)", valor_total as "Valor total do aluguel"
from Aluguel a 
join Cliente c on c.id_cliente = a.id_cliente
join Veiculo v on v.id_veiculo = a.id_veiculo;

create view K as 
SELECT v.modelo as "Modelo do veículo", sum(valor_total) as "Faturamento"
from Aluguel a 
join Veiculo v on v.id_veiculo = a.id_veiculo
GROUP by v.modelo;

create view L as 
SELECT c.nome AS "Nome do Cliente", a.valor_total AS "Compras de mais de R$5000"
FROM Aluguel a
JOIN Cliente c ON c.id_cliente = a.id_cliente
WHERE a.valor_total > 5000;


create view M as SELECT v.modelo AS "Modelo", v.marca AS "Marca", COUNT(a.id_veiculo) AS "Quantidade de Alugueis"
FROM Aluguel a
JOIN Veiculo v ON a.id_veiculo = v.id_veiculo
GROUP BY v.id_veiculo, v.modelo, v.marca
ORDER BY COUNT(a.id_veiculo) DESC
LIMIT 3;
